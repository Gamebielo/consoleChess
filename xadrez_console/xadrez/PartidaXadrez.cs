...
namespace xadrez
{
    class PartidaXadrez
    {
        ...
        public Peca execMovimento(Posicao origem, Posicao destino)
        {
            ...
            if (p is Peao)
            {
                // Diagonal e peca capturada é nula
                if (origem.coluna != destino.coluna && pecaCapturada == null)
                {
                    Posicao posP;
                    if (p.cor == Cor.Branca)
                    {
                        // Peça a ser capturada estará uma casa abaixo
                        posP = new Posicao(destino.linha + 1, destino.coluna);
                    }
                    else
                    {
                        // Peça a ser capturada estará uma casa acima
                        posP = new Posicao(destino.linha - 1, destino.coluna);
                    }
        ...
        public void realizaJogada(Posicao origem, Posicao destino)
        {
            ...
            if (estaEmXeque(jogadorAtual))
            {
                ...
                throw new TabuleiroException("Nãó é possível se colocar em xeque.");
            }
        ...
        public HashSet<Peca> pecasCapturadas(Cor cor)
        {
            ...
            foreach (var x in this.capturadas)
            {
                if (x.cor == cor)
                {
                    aux.Add(x);
                ...
                for (int i = 0; i < tab.linhas; i++)
                {
                    for (int j = 0; j < tab.colunas; j++)
                    {
                        if (mat[i, j])
                        {
                            ...
                            Posicao posDestino = new Posicao(i, j);
                            Peca pecaCapturada = execMovimento(origem, posDestino);
                            bool testeXeque = estaEmXeque(cor);
                            desfazMovimento(origem, posDestino, pecaCapturada);
                            if (!testeXeque)
                            {
                                return false;
                            }
